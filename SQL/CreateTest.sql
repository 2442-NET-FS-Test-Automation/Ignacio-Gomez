...
-- AVG() - get the average value stored in a column across many rows - skips nulls
-- MIN(), MAX()
...
SELECT UPPER(LastName) AS LaspUpper,
...
FROM dbo.Book AS b  
JOIN dbo.BookAuthor AS ba ON ba.BookId = b.BookId
...
-- Aggregates collapse all rows in a group into a single values
-- A bare column, would have possible values per group

-- LEFT JOIN - we want all records from the left table, and matching records from the rights
...
-- Use a subquery to filter against a computer valueato or set Dont need in the output
-- Realistically, you can usually use either - typically the Join will be easier
